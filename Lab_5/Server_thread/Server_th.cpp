...
void HandleClient(HANDLE hPipe, const string& filename) {
    ...
    while (true) {
        ...
        if (operationStr == "1") { // modification
            unique_lock<shared_mutex> lock(recordLock.mutex);
            ...
            shared_lock<shared_mutex> lock(recordLock.mutex);
...
int main() {
    try {
        ...
        cin >> num_processes;
        if (cin.fail() || num_processes <= 0) {
            throw runtime_error("Incorrect number of processes.");
        }

        wstring senderPath = L"Client_th.exe";
        ...
        vector<HANDLE> Pipes(num_processes);
        ...
        for (int i = 0; i < num_processes; ++i) {
            ...
            if (Pipes[i] == INVALID_HANDLE_VALUE) {
                cerr << "Pipe creation error: " << GetLastError() << endl;
                return 1;
            }
